...
test('native schemas expose only the allowed tools in each mode',()=>{
 ...
 assert.equal(toolDefinitions('plan').at(-1).name,'update_plan');assert.equal(toolDefinitions('agent').at(-1).name,'run_command');assert.deepEqual(toolDefinitions('agent',true),[]);
});
