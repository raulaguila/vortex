...
test('prompt stays concise, follows response language and enforces plan read-only tools',()=>{const plan=systemPrompt('plan','auto',[]);assert.ok(plan.includes("language of the user's message"));assert.ok(plan.includes('checklist'));assert.ok(!plan.includes('{"action":"write"'));assert.ok(plan.length<4000);assert.ok(systemPrompt('agent','en',[],false,'autonomous').includes('{"action":"write"'));});
