const {test}=require('node:test');const assert=require('node:assert/strict');const {PlanController,validateProposal,validatePlanState}=require('../dist/plan');const {turnActions}=require('../dist/turnProtocol');const {toolDefinitions}=require('../dist/actions');
...
test('workspace fingerprint changes with source edits and declines unsupported coverage',async t=>{
 const fs=require('node:fs/promises'),os=require('node:os'),path=require('node:path'),{planFingerprint}=require('../dist/planFingerprint');const root=await fs.mkdtemp(path.join(os.tmpdir(),'vortex-plan-hash-'));t.after(()=>fs.rm(root,{recursive:true,force:true}));await fs.writeFile(path.join(root,'a.txt'),'before');const a=await planFingerprint(root);assert.ok(a);await fs.writeFile(path.join(root,'a.txt'),'after');assert.notEqual(await planFingerprint(root),a);await fs.writeFile(path.join(root,'large.bin'),Buffer.alloc(2*1024*1024+1));assert.equal(await planFingerprint(root),null);
});
