...
(async()=>{
 ...
 try{
  ...
  await post(chat,{type:'activityUpdate',activity:{runId:'rejected',id:'validation-1',name:'modelValidation',status:'recovered',output:'Invalid read arguments corrected.',startedAt:Date.now(),endedAt:Date.now()}});assert.equal(await chat.locator('[data-activity-id="validation-1"] .activity-label').innerText(),'Request corrected; execution continued');
  await post(chat,{type:'status',busy:true,text:'Running command'});await post(chat,{type:'runProgress',progress:{runId:'live',phase:'tool',startedAt:Date.now(),phaseStartedAt:Date.now(),tool:{id:'cmd',name:'command'}}});await post(chat,{type:'commandOutput',runId:'stale',id:'cmd',stream:'stdout',text:'discard'});assert.equal(await chat.locator('.live-command').count(),0);await post(chat,{type:'commandOutput',runId:'live',id:'cmd',stream:'stdout',text:'Running tests…\n<script>never execute</script>'});await chat.locator('.live-command summary').click();assert.match(await chat.locator('.live-command pre').innerText(),/Running tests/);assert.equal(await chat.locator('.live-command script').count(),0);await chat.screenshot({path:path.join(output,'v080-live-command.png')});await post(chat,{type:'runEnd',requestId:'live',status:'complete'});assert.equal(await chat.locator('.live-command').count(),0);
