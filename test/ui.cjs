...
(async()=>{
 ...
 const server=createServer(async(req,res)=>{try{if(req.url==='/'||req.url==='/settings'){const isSettings=req.url==='/settings';const template=await fs.readFile(path.join(root,'media',isSettings?'settings.html':'sidebar.html'),'utf8');res.setHeader('Content-Type','text/html');res.end(renderSidebar(template,{cspSource:"'self'",nonce:'test',style:isSettings?'/settings.css':'/style.css',script:isSettings?'/settings.js':'/app.js',logo:'/vortex.svg',shared:'/shared.js',vendor:'/vendor.js',picker:'/picker.js'}));}else if(['/style.css','/settings.css','/app.js','/settings.js','/shared.js','/vendor.js','/picker.js','/vortex.svg'].includes(req.url)){res.setHeader('Content-Type',req.url.endsWith('.js')?'text/javascript':req.url.endsWith('.css')?'text/css':'image/svg+xml');res.end(await fs.readFile(path.join(root,'media',req.url.slice(1))));}else{res.statusCode=404;res.end();}}catch(e){res.statusCode=500;res.end();}});
