...
test('model timeout is validated and old profiles receive 120 seconds without losing other limits',async()=>{
 ...
 const manager=new ProviderManager(storage,{});assert.equal(manager.preferences().execution.modelTimeout,120);assert.equal(manager.preferences().execution.maxSteps,42);
 for(const modelTimeout of [0,-1,3601,1.5,'300',null])assert.throws(()=>parseRequest({type:'setExecution',requestId:'test',execution:{...defaultExecution(),modelTimeout}}));
 await manager.setExecution({...defaultExecution(),modelTimeout:600});assert.equal(new ProviderManager(storage,{}).preferences().execution.modelTimeout,600);
 assert.equal(parseRequest({type:'setExecution',requestId:'test',execution:{...defaultExecution(),modelTimeout:3600}}).execution.modelTimeout,3600);
});
test('all five chat adapters actually abort the request at the configured deadline',async()=>{
 ...
 const keepAlive=setInterval(()=>{},1000);try{await Promise.all(['openai','compatible','ollama','anthropic','gemini'].map(async kind=>{
  const manager=new ProviderManager({get:(key,fallback)=>key==='providers'?[provider(kind)]:key==='modelPreferences'?{execution:{...defaultExecution(),modelTimeout:1}}:fallback,update:async()=>{}},{get:async()=>''},stalled);const client=await manager.client('p'),start=Date.now();await assert.rejects(client.chat('m','system',[],new AbortController().signal),/modelo esgotado \(1s\)/);assert.ok(Date.now()-start>=900);assert.ok(Date.now()-start<3000);
...
test('timeout after HTTP headers retains the timeout error, and Stop takes precedence',async()=>{
 const keepAlive=setInterval(()=>{},1000);try{
  ...
  await assert.rejects(new Client(provider('compatible'),'',transport,undefined,1).chat('m','system',[],new AbortController().signal),/modelo esgotado/);
...
test('native streaming uses the configured deadline too',async()=>{
 const keepAlive=setInterval(()=>{},1000);try{
  ...
  await assert.rejects(new Client(provider('compatible'),'',transport,undefined,1).turn('m','system',[],new AbortController().signal,{tokens:16384,output:4096},[],()=>{}),/modelo esgotado/);
 }finally{clearInterval(keepAlive);}
});
