...
test('execution token budget stops before an unaffordable provider request',async()=>{
 const h=harness([{action:'read_file',path:'a'}]);h.agent.providers.preferences=()=>({conversation:{language:'auto'},execution:{maxSteps:20,commandTimeout:60,taskTimeout:1800,tokenBudget:1024}});
...
test('tool limit bounds a native batch without replaying unexecuted calls',async()=>{
 ...
 h.agent.providers.toolProtocol=()=> 'native';h.agent.providers.providers=()=>[{id:'p',kind:'openai'}];h.agent.providers.client=async()=>({turn:async()=>({text:'',calls:[{id:'a',name:'read_file',arguments:{path:'a'}},{id:'b',name:'read_file',arguments:{path:'b'}}]})});
...
test('greetings cannot invoke tools in any mode, even when the model requests writes',async()=>{
  for(const mode of ['ask','plan','agent'])for(const permission of ['supervised','autonomous']){
    const h=harness([{action:'write_file',path:'settings.json',content:'bad'},{action:'update_plan',items:[{id:'1',text:'invented',status:'completed'}]},{action:'run_command',command:'bad'}]);
...
test('social turn can recover with a direct reply and does not resume old tasks',async()=>{
  const h=harness([{action:'read_file',path:'README.md'},{action:'finish',text:'Oi! Como posso ajudar?'}]);
...
test('greeting plus concrete request still allows task tools',async()=>{
  const h=harness([{action:'read_file',path:'README.md'},{action:'finish',text:'Summary'}]);
...
test('repeated failing tools stop instead of cycling through twenty attempts',async()=>{
  const h=harness([{action:'read_file',path:'missing.txt'}]);let count=0;
...
test('mode and permission are supplied independently to the executor and prompt',async()=>{
 for(const permission of ['supervised','autonomous']){
  const h=harness([{action:'write_file',path:'file',content:'ok'},{action:'finish',text:'Done'}]);let received;
...
test('stop while the provider is running prevents dispatch and reports stopped',async()=>{
 const h=harness([{action:'write_file',path:'file',content:'bad'}]);
...
test('native calls execute sequentially and retain matching result IDs',async()=>{
 ...
 h.agent.providers.client=async()=>({turn:async(_model,_system,messages)=>{requests.push(structuredClone(messages));return ++count===1?{text:'',calls:[{id:'a',name:'read_file',arguments:{path:'a'}},{id:'b',name:'read_file',arguments:{path:'b'}}]}:{text:'Done',calls:[]};}});
...
test('native batch validation prevents any effects when one call is unauthorized',async()=>{
 ...
 h.agent.providers.client=async()=>({turn:async()=>({text:'',calls:[{id:'a',name:'read_file',arguments:{path:'a'}},{id:'b',name:'write_file',arguments:{path:'a',content:'bad'}}]})});
...
test('cancel during a native batch stops remaining calls and closes the transcript',async()=>{
 ...
 h.agent.providers.client=async()=>({turn:async()=>({text:'',calls:[{id:'a',name:'read_file',arguments:{path:'a'}},{id:'b',name:'read_file',arguments:{path:'b'}}]})});
...
test('standalone action announcements get one recovery, then real tools continue in the same mode',async()=>{
 for(const mode of ['ask','plan','agent'])for(const permission of ['supervised','autonomous']){
  const h=harness([{action:'finish',text:'Vou explorar os arquivos do workspace para entender o projeto.'},{action:'list_files',patterns:['src/**']},{action:'finish',text:'O projeto contém uma aplicação TypeScript.'}]);
  await h.run('O que pode me dizer sobre o projeto atual?',mode,permission);assert.equal(h.calls.length,3);assert.equal(h.tools.length,1);assert.equal(h.tools[0].action,'list_files');assert.equal(h.events.at(-1).status,'complete');assert.match(h.calls[1][2].at(-1).content,/not a new user request or authorization/);
 }
});
...
test('native announcement recovery retains tool result association',async()=>{
 ...
 h.agent.providers.toolProtocol=()=> 'native';h.agent.providers.providers=()=>[{id:'p',kind:'openai'}];
 h.agent.providers.client=async()=>({turn:async()=>++requests===1?{text:'I will inspect the project.',calls:[]}:requests===2?{text:'',calls:[{id:'read1',name:'read_file',arguments:{path:'README.md'}}]}:{text:'The project is an extension.',calls:[]}});
...
test('step exhaustion synthesizes observed results without tools and stays paused',async()=>{
 ...
 h.agent.providers.client=async()=>({chat:async(_model,system,messages)=>{calls++;if(system.startsWith('Summarize the current task')){assert.ok(messages.some(m=>m.toolResult?.status==='success'));return 'Inspected README; implementation is still pending.';}return JSON.stringify({action:'read_file',path:'README.md'});}});
...
test('compatibility validation returns the rejected reply and specific argument correction',async()=>{
 const h=harness([]);let count=0;const requests=[];
 h.agent.providers.client=async()=>({chat:async(_model,_system,messages)=>{requests.push(structuredClone(messages));return ++count===1?'{"action":"read_file","start_line":1}':count===2?'{"action":"read_file","path":"README.md"}':'README summary.';}});
 await h.run('Describe README','ask');
 const recovery=requests[1];assert.equal(recovery.at(-2).role,'assistant');assert.equal(recovery.at(-2).content,'{"action":"read_file","start_line":1}');assert.match(recovery.at(-1).content,/arguments.path is required/);assert.match(recovery.at(-1).content,/not a new user request or authorization/);
...
test('quoted false editor flag executes once as false and keeps the original native call',async()=>{
 for(const native of [false,true]){
  const h=harness([{action:'get_editor_context',include_selection:'false'},{action:'finish',text:'Open files inspected.'}]);
  if(native){let n=0;h.agent.providers.toolProtocol=()=> 'native';h.agent.providers.providers=()=>[{id:'p',kind:'compatible'}];h.agent.providers.client=async()=>({turn:async()=>++n===1?{kind:'tool_use',text:'',calls:[{id:'flag-call',name:'get_editor_context',arguments:{include_selection:'false'}}]}:{kind:'final',text:'Open files inspected.',calls:[]}});}
  await h.run('Which files are open?','ask');assert.deepEqual(h.tools,[{action:'get_editor_context',include_selection:false}]);assert.equal(h.events.at(-1).status,'complete');assert.equal(h.events.filter(e=>e.event?.activity?.name==='modelValidation').length,0);if(native)assert.equal(h.agent.messages.find(m=>m.toolCalls?.length).toolCalls[0].arguments.include_selection,'false');
 }
});
