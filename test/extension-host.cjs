...
(async()=>{
  ...
  try{
    ...
    scripted=[{action:'editor',selection:'true'},{action:'editor',selection:false},{action:'finish',text:'Editor recovery validated in Ask.'}];await frame.locator('#prompt').fill('Describe the files open in this workspace');await frame.waitForFunction(()=>!document.getElementById('send').disabled);await frame.locator('#send').click();await frame.locator('.message-body').filter({hasText:'Editor recovery validated in Ask.'}).waitFor();await frame.waitForFunction(()=>document.getElementById('stop').hidden);
