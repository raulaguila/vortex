...
(async()=>{
  ...
  }catch(error){if(app){const page=app.windows()[0];console.error('WORKBENCH AT FAILURE:',(await page.locator('body').innerText()).slice(-6000));await page.screenshot({path:path.join(root,'test-results','host-failure.png')}).catch(()=>{});for(const frame of page.frames())if(await frame.locator('#timeline').count())console.error('CHAT AT FAILURE:',await frame.locator('#timeline').innerText());}throw error;}finally{if(app)await app.close();await new Promise(resolve=>server.close(resolve));}
