...
export class AgentController extends AgentRuntime {
 constructor(providers:ProviderManager,post:(message:Response)=>void,sessions:SessionStore,reviews?:ReviewService,artifactsDirectory?:string,editor?:EditorContext,tracePath?:string){
  ...
  protected async execute(input:unknown,mode:Mode,root:string|undefined,signal:AbortSignal,permission:Permission='supervised',expected?:FileSnapshot):Promise<string>{
    ...
    if(a.action==='update_plan'||a.action==='read_tool_output')return super.execute(a,mode,root,signal,permission);
    if(a.action==='ask_user'){this.event('assistant',a.question);return askQuestion(a.question,a.options,signal);}
    ...
    if(a.action==='run_command'){
      ...
      if(permission==='autonomous'){
        ...
        if(await this.sandbox.available(image)){
          if(a.request_network&&await this.approval(()=>vscode.window.showWarningMessage('Allow network for this sandbox command?',{modal:true,detail:a.command},'Allow network'))!=='Allow network')throw new ApprovalDenied();
      ...
      signal.throwIfAborted();return JSON.stringify({execution_location:'host',output:await runCommand(a.command,cwd,signal,this.commandTimeout,1024*1024,(stream,text)=>this.commandOutput(stream,text))});
    }
