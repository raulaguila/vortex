...
export function searchPage(files:SearchFile[],query:string,regex:boolean,case_sensitive:boolean,signal:AbortSignal):Promise<{matches:{path:string;line:number;text:string}[];truncated:boolean}>{
  signal.throwIfAborted();
  return new Promise((resolve,reject)=>{
    const worker=new Worker(`const {workerData:d,parentPort}=require('node:worker_threads');try{
      const re=d.regex?new RegExp(d.query,d.case_sensitive?'':'i'):null,q=d.case_sensitive?d.query:d.query.toLocaleLowerCase();let matches=[],truncated=false;
    ...
    }catch{parentPort.postMessage({error:true});}`,{eval:true,workerData:{files,query,regex,case_sensitive},resourceLimits:{maxOldGenerationSizeMb:64}});
