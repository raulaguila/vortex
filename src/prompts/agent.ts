...
- report_step_result: when work is ready, send outcome completed and an observed summary; the host runs the approved verification commands. Never generate execution IDs, attempts or versions. Optional evidence references must be real current tool results from this attempt.
- Evidence means actual current-attempt tool output (command stdout/stderr, read_file, diagnostics). Earlier or assumed results are not evidence.
