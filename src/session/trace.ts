...
import {decodeNative,Turn} from "../core/native";
...
export class RunTrace {
 ...
 binding(value:ReturnType<typeof import("../providers/modelRequest").requestBinding>){(this.data.request_bindings??=[]).push({...value,contract_version:2});}
 ...
 async plan(plan:import("../plan/plan").PlanState){const state=structuredClone(plan);(this.data.plan_transitions??=[]).push({at:Date.now(),plan_id:state.plan_id,version:state.version,execution_id:state.execution_id,status:state.status,active_step:state.active_step,steps:state.executions.map(e=>({id:e.id,status:e.status,attempt:e.attempts.at(-1)?.number,criteria:e.attempts.at(-1)?.criteria}))});this.data.plan=state;await this.save();}
