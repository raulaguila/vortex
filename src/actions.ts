...
import {ModelProposal,ModelStepReport,normalizeProposal} from './planContract';

export type Action =
  | {action:'finish';text:string}
  | {action:'list_files';patterns?:string[];exclude_patterns?:string[];cursor?:string;offset?:number;limit?:number}
  | {action:'read_file';path:string;start_line?:number;end_line?:number}
  | {action:'search_files';query:string;patterns?:string[];exclude_patterns?:string[];cursor?:string;offset?:number;case_sensitive?:boolean;regex?:boolean}
  ...
  | {action:'ask_user';question:string;options?:string[];recommended_option?:string}
...
export const registry:Record<Action['action'],Entry>={
 ...
 list_files:define('Discover workspace paths using OR-combined glob patterns; exclusions remove matches. Use for project structure or locating files before reading. Returns paths, not contents, with coverage and next_cursor; continue with the same filters. Default page: 100; maximum: 500. Never treat a partial page as the entire workspace.',object({patterns,exclude_patterns:exclusions,cursor:string('Opaque next_cursor from the previous page. Repeat the same filters; omit to restart.',36,1),offset:integer(0,10000,'File offset from next_offset. Default: 0.'),limit:integer(1,500,'Maximum paths returned. Default: 100.')},[]),{patterns:['src/**']},'read',modes,'Listing files'),
 ...
 search_files:define('Search text in workspace files matching OR-combined patterns. Literal and case-insensitive by default; regex is optional. Initial offset pages files; each page reads up to 100 files. Use next_cursor with the same query and filters to continue within a truncated file. Inspect coverage and skipped_reasons: no matches only describes scanned files.',object({query:string('Text or regex',300,1),patterns,exclude_patterns:exclusions,cursor:string('Opaque next_cursor from the previous page. Repeat query and filters; omit to restart.',36,1),offset:integer(0,10000,'File offset from next_offset. Default: 0.'),case_sensitive:{type:'boolean',description:'Match letter case. Default: false.'},regex:{type:'boolean',description:'Interpret query as regex. Default: false.'}},['query']),{query:'function',patterns:['src/**']},'read',modes,'Searching files'),
 ...
 ask_user:define('Pause for one focused question when missing information or a user decision is necessary. Optional choices must be clear and mutually exclusive. recommended_option optionally names exactly one offered choice; omit when there is no recommendation. Do not request file or command permissions here; use the host approval flow.',object({question:string('One self-contained question.',1000,1),options:{type:'array',items:string('Choice',160,1),minItems:2,maxItems:5},recommended_option:string('Exact text of one offered choice to recommend; never preselects or submits it.',160,1)},['question']),{question:'Which behavior do you want?',options:['Option A','Option B']},'interaction',modes,'Waiting for your answer'),
 ...
 propose_plan:define('Propose ordered implementation steps for approval. Include exact files and operations, necessary commands, and observable acceptance criteria. The host assigns all IDs. Dependencies are optional 1-based earlier step numbers. Command criteria must use a known command that exits nonzero on failure; cwd defaults to dot. Human criteria require review. Approval authorizes the displayed scope. Return this call alone.',object({objective:string('Overall objective',4000,1),steps:{type:'array',minItems:1,maxItems:50,items:object({title:string('Short step title',300,1),objective:string('Step objective',4000,1),depends_on:{type:'array',items:integer(1,49),maxItems:49},files:{type:'array',maxItems:100,items:object({path:file,operation:{type:'string',enum:['create','edit','delete']}})},commands:{type:'array',maxItems:30,items:object({command:string('Exact necessary command',20000,1),cwd:file,request_network:{type:'boolean'}},['command'])},criteria:{type:'array',minItems:1,maxItems:20,items:object({description:string('Expected observable result',1000,1),verification:{type:'string',enum:['command','human']},command:string('Known check that fails with nonzero exit code',20000,1),cwd:file},['description','verification'])}},['title','objective','criteria'])}}),{objective:'Fix addition',steps:[{title:'Fix and test sum',objective:'Make sum add its arguments',files:[{path:'sum.js',operation:'edit'}],criteria:[{description:'Existing addition tests pass',verification:'command',command:'npm test'}]}]},'interaction',['plan','agent'],'Proposing plan'),
 report_step_result:define('Report completed, blocked or failed for the active step, with an observed summary and any remaining issues. The host binds this response to its request and runs the approved checks. Do not send execution IDs, step IDs, versions or attempts. Optional evidence references must come from current tool results. A completion claim does not advance the plan until validation succeeds. Return this call alone.',object({outcome:{type:'string',enum:['completed','blocked','failed']},summary:string('Observed work or blocker',4000,1),evidence:{type:'array',maxItems:20,items:object({criterion_id:string('Criterion ID supplied by the host',80,1),tool_call_ids:{type:'array',maxItems:200,items:string('Observed evidence ID',100,1)}})},remaining_issues:{type:'array',maxItems:20,items:string('Unresolved issue',1000,1)}},['outcome','summary']),{outcome:'completed',summary:'Updated addition; ready for the approved checks.'},'interaction',['agent'],'Validating step'),
...
export function toolSelectionInstructions(mode:Mode,activeStep=false):string{
 return allowedActions(mode).filter(name=>activeStep||name!=='report_step_result').filter((name):name is Exclude<Action['action'],'finish'>=>name!=='finish').map(name=>'- '+name+': '+toolPurposes[name]).join('\n');
}
export function toolInstructions(mode:Mode,activeStep=false):string{return allowedActions(mode).filter(name=>activeStep||name!=='report_step_result').map(name=>catalog[name]).join('\n');}
...
export function validateAction(value:unknown,mode:Mode,conversationOnly=false):Action{
 ...
 if(name==='ask_user'&&args.recommended_option!==undefined&&(!Array.isArray(args.options)||!args.options.includes(args.recommended_option)))throw new Error('recommended_option must exactly match an offered option.');
 if(name==='read_file'&&Number(args.end_line??Infinity)<Number(args.start_line??1))throw new Error('Invalid line range.');
 if(name==='propose_plan')try{validateProposal(normalizeProposal(args as unknown as ModelProposal));}catch(error){throw new Error('Invalid propose_plan arguments: '+(error as Error).message);}
...
export function toolDefinitions(mode:Mode,conversationOnly=false,activeStep=false):ToolDefinition[]{return allowedActions(mode,conversationOnly).filter(name=>name!=='finish'&&(activeStep||name!=='report_step_result')).map(name=>({name,description:registry[name].description,inputSchema:registry[name].schema}));}
