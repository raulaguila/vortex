...
export async function runIsolated(manager:ProviderManager,model:ModelRef,prompt:string,mode:Mode,execute:RuntimeHost['execute'],signal:AbortSignal,options:{maxRounds?:number;maxToolCalls?:number;turns?:{prompt:string;mode:Mode}[]}={}){
 ...
 try{await runtime.start({type:'start',requestId:randomUUID(),prompt,mode,permission:'supervised',model});for(const turn of options.turns||[]){if(saved?.pendingTool)break;await runtime.start({type:'start',requestId:randomUUID(),prompt:turn.prompt,mode:turn.mode,permission:'supervised',model});}}
