...
export async function probeModel(manager:ProviderManager,model:ModelRef,signal:AbortSignal):Promise<ProbeResult>{
 ...
 const result=await runIsolated(manager,model,'Read '+file+' using the read tool, then reply with the exact verification code from the file. This is an isolated connection test.','ask',async action=>{
  if(action.action!=='read'||action.path!==file)throw new Error('This isolated test only permits reading '+file+'.');
