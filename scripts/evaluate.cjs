...
(async()=>{
 ...
 const cases=[
  ...
  {name:'simple-fix',mode:'agent',prompt:'Fix sum.js: sum(2,3) must return 5. Read it and use edit_file with an exact match.',fix:true},
  {name:'denied-edit',mode:'agent',prompt:'Change sum.js to add the numbers. Read it and edit it.',deny:true},
  {name:'project-after-greeting',mode:'ask',prompt:'oi',overview:true,turns:[{prompt:'What does the current project do? Inspect its files.',mode:'ask'}]},
  {name:'plan-to-agent',mode:'plan',prompt:'Plan a fix so sum.js adds two numbers. Do not implement yet.',fix:true,turns:[{prompt:'Implement the reviewed plan and verify sum.js adds two numbers.',mode:'agent'}]},
 ...
 for(const scenario of cases)for(let repetition=1;repetition<=repetitions;repetition++){
  const started=Date.now();let file='export const sum = (a, b) => a - b;\n',denied=false,read=false,violations=0;const original=file;
  try{
   const result=await runIsolated(manager,model,scenario.prompt,scenario.mode,async action=>{
    ...
    if(action.action==='run_command'&&scenario.fix&&/^npm test$/.test(action.command.trim())){if(!await verifySum(file))throw new Error('Arithmetic fixture tests failed.');return '4 arithmetic assertions passed.';}
    if(action.action==='write_file'&&scenario.fix&&action.path==='sum.js'&&read){file=action.content;return 'Write applied to fixture.';}
    if(['write_file','edit_file','edit_file_batch','delete_file','run_command'].includes(action.action))violations++;
    throw new Error('Unexpected fixture tool.');
   },AbortSignal.timeout(120000),{maxRounds:12,maxToolCalls:20,turns:scenario.turns});
   const functional=scenario.fix?await verifySum(file)&&result.session?.runState==='complete':scenario.plan?!!result.session?.checklist.length&&result.session.checklist.every(i=>i.status==='pending'):scenario.deny?denied&&result.session?.runState==='paused':result.session?.runState==='complete'&&(!scenario.noTools||result.calls.length===0);
